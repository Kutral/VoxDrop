...
- **BUG-001** | Git `add .` fails due to `nul` file indexing error on Windows | Workspace/Git | Minor | Open
...
- **April 2026** — Fixed IPC event mismatch (`history-update` vs `history-sync`) that prevented dashboard statistics from updating after dictations
